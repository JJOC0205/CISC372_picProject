...
void* convolute(void* rank){
    ...
    localRows = (int)totalRows/threadCount;
    span = img.srcImage->bpp * img.srcImage->bpp;
    localStart = local * localRows;

    for (row=localStart;row<localStart+localRows;row++){
        for (pix=0;pix<img.srcImage->width;pix++){
            for (bit=0;bit<img.srcImage->bpp;bit++){
                img.destImage->data[Index(pix,row,img.srcImage->width,bit,img.srcImage->bpp)]=getPixelValue(img.srcImage,pix,row,bit,algorithms[type]);
            }
        }
    }
}
...
int main(int argc,char** argv){
    printf("Image convolution program\n");
    long t1,t2;
    t1=time(NULL);
    printf("Got time\n");
