...
void* convolute(void * rank){
    int localRows, my_start, bit, span, pix;
    long localRank = (long) rank;
    int rows = img.destImage->height;
    span=img.srcImage->bpp * img.srcImage->bpp;

    //calculate how many rows each thread will process
    localRows = (int) rows / threadCount;
    my_start = (int) localRank * localRows;
    if((int)localRank==threadCount-1){
        localRows+=rows % threadCount;
    }

    //process the given rows for the thread
    for(int row = my_start; row<my_start+localRows; row++){
...
int main(int argc,char** argv){
    ...
    for(thread=0; thread<threadCount; thread++){
        ...
        pthread_create(&threads[thread], NULL, &convolute, (void *) thread); 
    }
    //join the threads
    for(thread=0; thread<threadCount; thread++){
